...
func (dal *ListenerDAL) CreateListener(ctx context.Context, listener *models.Listener) error {
	...
	if err != nil {
		logger.Error(logLevel, "Failed to marshal listener config: ", err)
		return fmt.Errorf("failed to marshal listener config: %w", err)
	}

	logging, err := json.Marshal(listener.Logging)
	if err != nil {
		logger.Error(logLevel, "Failed to marshal listener logging: ", err)
		return fmt.Errorf("failed to marshal listener logging: %w", err)
	}

	logger.Debug(logLevel, "Creating listener: "+listener.ID.String())
	...
	if err != nil {
		logger.Error(logLevel, "Failed to create listener: ", err)
...
func (dal *ListenerDAL) GetListenerById(ctx context.Context, lId string) (models.Listener, error) {
	logger.Debug(logLevel, "Getting listener by id: "+lId)
...
func (dal *ListenerDAL) GetAllListeners(ctx context.Context) ([]models.Listener, error) {
	logger.Debug(logLevel, "Getting all listeners")
	...
	if err != nil {
		logger.Error(logLevel, "Failed to get listeners: ", err)
...
func (dal *ListenerDAL) DeleteListener(ctx context.Context, lid string) error {
	logger.Debug(logLevel, "Deleting listener: "+lid)
...
func (dal *ListenerDAL) UpdateListener(ctx context.Context, lid string, updates map[string]any) error {
	...
	logger.Debug(logLevel, "Updating listener: "+lid)
	...
	if err != nil {
		logger.Error(logLevel, "Failed to retrieve affected rows: ", err)
		return fmt.Errorf("failed to retrieve affected rows")
	}
	if rowsAffected == 0 {
		logger.Warn(logLevel, "No rows were updated")
		return fmt.Errorf("no listener found with id: %s", lid)
	}

	logger.Debug(logLevel, "Rows Affected:", rowsAffected)
	return nil
}
