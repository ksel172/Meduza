...
func (a *TestHTTPAgent) Authenticate(t *testing.T, listenerID string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal c2request: %v", err)
	}
	...
	if resp.StatusCode != http.StatusAccepted {
		return fmt.Errorf("unexpected status code, expected: %d, got: %d", http.StatusAccepted, resp.StatusCode)
	}
...
func (a *TestHTTPAgent) Register(t *testing.T) error {
	c2request := models.C2Request{
	...
	if err != nil {
		return fmt.Errorf("failed to encrypt c2request: %v", err)
	}
	...
	if resp.StatusCode != http.StatusCreated {
		return fmt.Errorf("unexpected status code, expected: %d, got: %d", http.StatusCreated, resp.StatusCode)
	}

	// The server returns the agent ID
	var agentID string
	if err := json.NewDecoder(resp.Body).Decode(&agentID); err != nil {
		return fmt.Errorf("failed to decode agent ID from response: %v", err)
	}
...
func (a *TestHTTPAgent) GetTasks(t *testing.T) error {
	c2request := models.C2Request{
		AgentID: a.ID,
		Reason:  models.Task,
	}
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("unexpected status code, expected: %d, got: %d", http.StatusOK, resp.StatusCode)
	}

	return nil
}

func (a *TestHTTPAgent) SendResponse(t *testing.T) error {
	c2request := models.C2Request{
		AgentID: a.ID,
